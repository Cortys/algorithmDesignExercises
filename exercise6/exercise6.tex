		...
		\Function{PageMigration}{$\langle \nu_1, \dots, \nu_n \rangle, \langle \sigma_1, \dots, \sigma_t \rangle, D, d$}
			\For{$s \gets 1 \ato n$} \Comment{$\mathcal{O}(n)$}
				\For{$r \gets 1 \ato n$} \Comment{$\mathcal{O}(n^2)$}
					\State{$p[t + 1, s, r] \gets d(\nu_s, \nu_r) \cdot D$} \Comment{$\mathcal{O}(n^2 \cdot 1)$}
				\EndFor\
			\EndFor\
			\Statex\
			\For{$i \gets t \ato 1$} \Comment{$\mathcal{O}(t)$}
				...
				\For{$s \gets 1 \ato n$} \Comment{$\mathcal{O}(t\cdot n)$}
					\For{$r \gets 1 \ato n$} \Comment{$\mathcal{O}(t \cdot n^2)$}
						\State{$p_0 \gets p[i + 1, s, r] + d(\nu_s, \sigma_i)$}
						\State{$p_1 \gets p[i + 1, \pi_i, r] + d(\nu_s, \sigma_i) \cdot D$}
						\State{$p_2 \gets p[i + 1, s, \pi_i] + d(\sigma_i, \nu_r) \cdot D$}
